<template>
    <UModal>
        ...
        <template #header>
            <h3 class="text-base font-semibold">Загрузка по URL</h3>
        </template>
        <template #body>
            <UInput v-model="sourceUrl" placeholder="https://example.com/file.zip" autofocus
...
async function handleSubmit(close: () => void) {
    ...
    try {
        ...
        toast.add({ title: 'Загрузка по URL успешно начата!', description: 'Файл появится в списке после завершения.', color: 'success' });
        emit('submitted');
        sourceUrl.value = '';
        close();
    } catch (e: any) {
