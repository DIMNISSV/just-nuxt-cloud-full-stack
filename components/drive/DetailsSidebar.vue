<template>
  <!-- Панель отображается только если есть выбранный узел -->
  <aside v-if="node" class="p-4 border-l bg-white w-80 flex-shrink-0">
    ...
    <div class="space-y-4">
      <!-- Иконка и Редактируемое имя -->
      <div class="text-center py-4">
        <div class="text-6xl mb-4 flex justify-center" :class="{
          'text-yellow-500': node.type === 'FOLDER',
          'text-gray-500': node.type === 'FILE',
        }">
          <Icon :name="node.type === 'FOLDER' ? 'heroicons:folder-20-solid' : 'heroicons:document-20-solid'" />
        </div>
        <!-- Используем новый компонент для имени -->
...
const props = defineProps<{
  node: StorageNode | null; // Принимаем узел (или null)
}>();
