...
export default defineEventHandler(async (event) => {
    ...
    if (!uuid) throw createError({ statusCode: 400, message: 'Некорректный UUID узла' });

    const node = await prisma.storageNode.findUnique({ where: { uuid } });

    if (!node || node.ownerId !== user.userId) throw createError({ statusCode: 404, message: 'Узел не найден' });
    if (node.status !== NodeStatus.PENDING) throw createError({ statusCode: 409, message: 'Неверный статус узла' });

    const tempS3Key = node.s3Key;
    if (!tempS3Key) throw createError({ statusCode: 500, message: 'У узла отсутствует временный ключ S3' });

    const isUploaded = await objectExists(tempS3Key);
    if (!isUploaded) {
        await prisma.storageNode.delete({ where: { id: node.id } });
        throw createError({ statusCode: 400, message: 'Файл не был загружен в S3.' });
    }
