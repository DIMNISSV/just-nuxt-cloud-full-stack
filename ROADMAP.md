...
## **Спринт 3: Серверные загрузки (URL и Торренты)**

**Цель:** Реализовать механизмы, при которых сервер сам скачивает контент по инструкции от клиента (URL, magnet-ссылка).
...
*   **Коммит 3.2: `feat(api): implement Torrent download workflow`**
    *   Добавить торрент-клиент (Transmission) в `docker-compose.yml`.
    *   Создать новый API-эндпоинт `POST /api/v1/assets/download-from-torrent`.
    *   **Payload:** `{ magnetLink: string, assetType: AssetType, folderId?: number, selectFiles?: string[] }`.
    *   **Логика:**
        1.  Создает "виртуальную" родительскую задачу (возможно, в отдельной модели `TorrentJob`) для отслеживания всей раздачи (если торрент содержит много файлов).
        2.  Ставит задачу в очередь BullMQ (`process-torrent-job`), передавая `magnetLink` и опциональные `selectFiles`.
        3.  Возвращает ID задачи для отслеживания.

*   **Коммит 3.3: `feat(worker): add torrent processing support`**
...
*   **Коммит 3.4: `refactor(ui): update upload UI with server-side options`**
    *   На страницах `/account/storage` и/или новой `/account/media-assets` добавить UI-элементы (вкладки/кнопки) для загрузки по URL и по Magnet-ссылке, которые будут вызывать соответствующие новые API-эндпоинты.
